...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error('[submit-practice]', err)
    return json({ error: '提交失败，请稍后重试' }, 400)
  }
})
...
async function resolveRestaurant(
  ...
  if (payload.selected_poi) {
    ...
    const displayLabel = poi.amap_small_category ?? poi.amap_mid_category ?? null
    ...
    const { data, error } = await admin
      ...
      .insert({
        ...
        status: 'active',
        discovered_from: 'practice',
        search_text: [displayName, poi.address_text, poi.district_name, displayLabel]
          .filter(Boolean)
          .join(' '),
      })
      .select('id')
